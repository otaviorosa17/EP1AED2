...
/*  Este programa fornece funcoes que calculam 8 metricas de grafos, 
    bem como alguns testes executados na main(), algumas funcoes de suporte
    ja foram fornecidas no codigo do arquivo "completeERenomeie.c".
*/
...
void vizinhosEmComum(Grafo* g, int v, int* vizinhos){
  ...
  for(atual = 0; atual<g->numVertices; atual++) { // loop que percorre todo vertice do grafo "linha" por "linha"
    ...
    for(i = 0; i<g->numVertices; i++) { // loop que percorre a "linha" de vizinhos (1) ou nao vizinhos (0) do vertice atual
        if (g->matriz[atual][i] && g->matriz[v][i] == 1) { // verifica se o vizinho do vertice atual tambem e vizinho de v
            vizinhosEmComum++; // soma todos os vizinhos em comum entre o atual e v
        }
    }
    vizinhos[atual] = vizinhosEmComum; // preenche o array fornecido atraves do parâmetro da funcao com os respectivos numeros de vizinhos em comum
  }
}
...
void coeficienteDeJaccard(Grafo* g, int v, float* coeficientes){
  ...
  for(atual = 0; atual<g->numVertices; atual++) { // loop que percorre todo vertice do grafo "linha" por "linha"
    ...
    orCounter = 0;
    for(i = 0; i<g->numVertices; i++) { // loop que percorre a "linha" de vizinhos (1) ou nao vizinhos (0) do vertice atual
        if (g->matriz[atual][i] || g->matriz[v][i] == 1) { // verifica se pelo menos um dos vertices (atual ou v) e vizinho de i
          orCounter++;
          if (g->matriz[atual][i] && g->matriz[v][i] == 1) { // verifica se o vertice atual e v sao vizinhos de i
            andCounter++;
          }
        }
    }
    if (orCounter == 0) coeficientes[atual] = -1; // cobre o caso em que o denominador (orCounter) e igual a zero
    else coeficientes[atual] = (float) andCounter/orCounter; // preenche o array fornecido atraves do parâmetro da funcao com os respectivos coeficientes de cada vertice
  }
}
  ...
  for(atual = 0; atual<g->numVertices; atual++) { // loop que percorre todo vertice do grafo "linha" por "linha"
    ...
    for(i = 0; i<g->numVertices; i++) { // loop que percorre a "linha" de vizinhos (1) ou nao vizinhos (0) do vertice atual
      if (g->matriz[atual][i] && g->matriz[v][i] == 1) { // verifica se i e um vizinho em comum entre o v e o atual
        vizinhosI = 0;
        vizinhosI = retornaGrauDoVertice(g, i); // calcula o número de vizinhos de i
        somatorio = somatorio + (1/(float)vizinhosI); // calcula o valor de um elemento do somatorio da formula de alocacao de Recursos e acumula o valor na variavel somatorio
      }
    }
    coeficientes[atual] = somatorio; // preenche o array fornecido atraves do parâmetro da funcao com os respectivos coeficientes de cada vertice
  }
...
/* Funcao que calcula a Similaridade Cosseno do vertice v em relacao a cada um dos vertices do grafo*/
void similaridadeCosseno(Grafo* g, int v, float* coeficientes){
  ...
  for(atual = 0; atual<g->numVertices; atual++) { // loop que percorre todo vertice do grafo "linha" por "linha"
    ...
    raiz = sqrt((float)grauV*grauAtual); // guarda o valor do denominador da formula "Similaridade Cosseno"
    if (raiz==0) coeficientes[atual] = -1; // cobre o caso em que o denominador (raiz) e igual a zero
    else coeficientes[atual] = ((float)vizinhosEmComumV[atual])/raiz; // preenche o array passado como parâmetro da funcao com os coeficientes de cada vertice do grafo
  }
  free(vizinhosEmComumV); // libera memoria do array "vizinhosEmComumV" anteriormente alocado
}


/* Coeficiente de Dice */
/* Funcao que calcula o Coeficiente de Dice do vertice v em realcao a cada um dos vertices do grafo*/
void coeficienteDeDice(Grafo* g, int v, float* coeficientes){
  ...
  for(atual = 0; atual<g->numVertices; atual++) { // loop que percorre todo vertice do grafo "linha" por "linha"
    ...
    if (soma==0) coeficientes[atual] = -1; // cobre o caso em que o denominador (soma) e igual a zero
    else coeficientes[atual] = ((float)2*vizinhosEmComumV[atual])/soma; // preenche o array passado como parâmetro da funcao com os coeficientes de cada vertice do grafo
  }
  free(vizinhosEmComumV); // libera memoria do array "vizinhosEmComumV" anteriormente alocado
}


/* Funcao auxiliar que retorna o valor mínimo entre dois inteiros*/
int min(int a, int b) {
  return (a<b) ? a : b;
}
...
/* Funcao auxiliar que retorna o valor maximo entre dois inteiros*/
int max(int a, int b) {
  return (a>b) ? a : b;
}
...
/* Hub Promoted Index */
/* Funcao que calcula a medida HPI do vertice v em relacao a cada um dos vertices do grafo*/
void HPI(Grafo* g, int v, float* coeficientes){
  ...
  for(atual = 0; atual<g->numVertices; atual++) { // loop que percorre todo vertice do grafo "linha" por "linha"
    ...
    if (minimo==0) coeficientes[atual] = -1; // cobre o caso em que o denominador (minimo) e igual a zero
    else coeficientes[atual] = ((float)vizinhosEmComumV[atual])/minimo; // preenche o array passado como parâmetro da funcao com os coeficientes de cada vertice do grafo
  }
  free(vizinhosEmComumV); // libera memoria do array "vizinhosEmComumV" anteriormente alocado
}
...
/* Hub Depressed Index */
/* Funcao que calcula a medida HDI do vertice v em relacao a cada um dos vertices do grafo*/
void HDI(Grafo* g, int v, float* coeficientes){
  ...
  for(atual = 0; atual<g->numVertices; atual++) { // loop que percorre todo vertice do grafo "linha" por "linha"
    ...
    if (maximo==0) coeficientes[atual] = -1; // cobre o caso em que o denominador (maximo) e igual a zero
    else coeficientes[atual] = ((float)vizinhosEmComumV[atual])/maximo; // preenche o array passado como parâmetro da funcao com os coeficientes de cada vertice do grafo
  }
  free(vizinhosEmComumV); // libera memoria do array "vizinhosEmComumV" anteriormente alocado
}
